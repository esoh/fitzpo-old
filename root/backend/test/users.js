                ...
                it('should fail with status code 409', function(done){
                    chai.request(expressURL)
                        ...
                        .end(function(err, res){
                            ...
                            expect(res.body.error.errorType).to.equal('DuplicateKeyError')
                            expect(res.body.error).to.have.property('message')
                            expect(res.body.error.message).to.equal('Username is taken')
                            done()
                        })
                })
            })
                ...
                it('should fail with status code 409', function(done){
                    chai.request(expressURL)
                        ...
                        .end(function(err, res){
                            ...
                            expect(res.body.error.errorType).to.equal('DuplicateKeyError')
                            expect(res.body.error).to.have.property('message')
                            expect(res.body.error.message).to.equal('Email is in use')
                            done()
                        })
                })
            })
